...
if(!file.exists("data/wp_pages_complete.RDS")){
    ...
    wm_full=readRDS("data/wm_full.RDS") %>% select(flood,deathtoll) %>% unique()
...
```
```{r}
# ggplot(wp_pages_complete, aes(x=local,n_views_mean, fill=local))+
#   geom_boxplot()+
#   scale_y_log10()
...
```{r}
# ggplot(wp_pages_complete,aes(x=n_edits,y=n_views_tot, col=local))+
#   geom_point()+
...
```{r}
# ggplot(wp_pages,aes(x=deathtoll,y=n_views_f3months, col=local))+
#   geom_point()+
#   scale_x_log10()+scale_y_log10()+
#   geom_smooth(method="lm")
# 
# 
# ggplot(wp_pages,aes(x=deathtoll,y=n_views_tot, col=local))+
#   geom_point()+
#   scale_x_log10()+scale_y_log10()+
#   geom_smooth(method="lm")
```
