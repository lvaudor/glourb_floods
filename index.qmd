...
Fig. 6 shows the topics identified by the Reinert classification method. Based on the over-represented terms in each cluster we propose the following labelling of the classes: 'governance' for cluster 1 ("government","minister","president", "response", etc.), 'relief' for cluster 2 ("person", "family","evacuate","food","rescue"), 'hydrology' for cluster 3 ("water","level","meter","flow","reach","rise","cubic","reservoir","dam"),'damage' for cluster 4 ("road","house","destroy","damage") 'weather' for cluster 5 ("rain","heavy","rainfall","precipitation","occur"), and 'climate' for cluster 6 ("pressure", "warm", "air","mediterranean","climate",.
...
```{r, fig.width=10}
#| label: fig-fig9
#| fig-cap: "Crossed correlations between metrics (events and pages characteristics), displayed as a) a correlation matrix plot of metrics and b) a factorial map of metrics derived from a Principal Components Analysis. Non-significant correlations are marked with a cross"
wp_pages_complete=readRDS("data/wp_pages_complete.RDS")
...
Fig. 9 shows a correlation matrix (Fig. 9.a) and a principal component analysis (PCA) factorial map of a variety of metrics describing events' characteristics (year, deathtoll, HDI, population, density) and Wikipedia's articles characteristics (length, complex_topics, local, curation). All these metrics were transformed to ranks to limit the effect of non-standard distributions prior to calculating correlations and running the PCA analysis. The PCA first factorial map accounts for 37.8% of total variance. Principal Component 1 is mainly associated with country-related variables (population, density, HDI) but also events' deathtoll, which is positively (and significantly, see Fig. 9.a) correlated to population and density while it is negatively (and significantly) correlated to HDI, which is consistent with floods causing more damage in vulnerable countries. Principal Component 2 is mainly associated with articles' characteristics such as length, complex_topics, curation, views). Number of views is correlated positively and significantly to all other variables: it tends to be higher when event's deathtoll is high, impacted country is dense,populated and with a high HDI and when the article is written in a language local to the event, long, curated, and topically rich. The number of views of Wikipedia articles about floods also tends to increase with time. On the other hand, curation is negatively (and significantly) correlated to year, deathtoll and length, which suggests that recent and deadlier events correspond to longer, less careful reports on Wikipedia. Similarly, the proportion of complex topics dealt with by articles is negatively (and significantly) correlated to year, deathtoll, population, and density. This suggests that articles that deal with deadlier events (which also tend to occur in dense and populated countries) display a general tendency to be more narrative in their contents than, maybe, critical.
