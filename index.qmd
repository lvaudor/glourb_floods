...
wp_pages_complete=readRDS("data/wp_pages_complete.RDS") %>% 
  ...
  left_join(countries %>%
              select(country_label,HDI,population,area)) %>% 
  mutate(year=lubridate::year(date)) %>% 
  filter(year>2015) %>%   
...
datarf=wp_pages_complete %>% select(
                                    local,curation,
                                    length,
                                    deathtoll,
                                    HDI,
...
myrf=rfsrc(mean_views_f3months~., data=datarf,ntree=1000)
gg_md=gg_minimal_depth(myrf)
```
...
topvars=myvimp$importance %>% sort(decreasing=TRUE) %>% names()
...
for(i in 1:7){
  if(names(gg_p)[i]=="local"){
    ...
    plots[[i]]=ggplot(dat, aes(x=x,y=y))+
      geom_path(col=col_WD)+
      geom_point(col=col_WD)+
      geom_line(aes(x=x,y=y-se),col=col_DFO)+
      geom_line(aes(x=x,y=y+se),col=col_DFO)
  }
  if(names(gg_p)[i] %in% c("population", "length", "density","deathtoll")){
    plots[[i]]=plots[[i]]+
      scale_x_log10()
  }
  plots[[i]]=plots[[i]]+
...
ggpubr::ggarrange(plots[[1]],plots[[2]],plots[[3]],plots[[4]],plots[[5]],plots[[6]],
...
The difficulty we faced with the spatial information available in Wikidata regarding floods demonstrates it is not, as such and on its own, a really convenient and rich data source for spatial analysis, as it is not primarily designed for this purpose. This limitation could be overcome by the use of other data sources, such as OpenStreetMap, which is more spatially oriented and linked to Wikidata. However, although OpenStreetMap would be a suitable source of spatial data for Wikidata items corresponding to physical objects, it does not deal with events. Whatever the database's structure though, providing spatial information regarding an event (such as a flood) is actually not a trivial task, as it requires the definition (through time) of the event's spatial extent, location(s), and/or impact area(s).
