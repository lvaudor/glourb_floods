...
Frontiers in Big Data Journal of flood risk management Natural Hazards Progress in Human Geography GeoJournal

# Abstract

This study explores the use of Wikidata and Wikipedia as viable sources for flood data analysis. We evaluate the reliability and scope of these platforms despite inherent challenges such as geographical biases and inconsistencies in data. Our findings indicate that Wikidata and Wikipedia are particularly valuable for providing not only detailed textual descriptions and social context but also insights into the public’s interest and engagement with flood events. By juxtaposing this dataset with an established flood database, we investigate the conditions and extent to which flood-related pages are generated on these platforms. Our analysis reveals that the creation, content, and diffusion of Wikidata and Wikipedia entries are influenced by factors including geographical location, social context, and event's severity. This underscores the potential of these platforms as complementary tools in disaster management and research, offering a unique perspective.
...
Wikidata; Wikipedia; Crowd-sourced knowledge; Social context; Public interest; flood; disaster management;
...
- 1)  collect and assess the information provided by the Wikimedia initiative (in particular, Wikidata and Wikipedia) on flood events.

- 2)  compare this data, which is crowdsourced, to an institutionnally curated database on the same topic to estimate its validity and interest.
...
To better understand the non-linear relationship between quantitative variables and a set of non-independent descriptors, we ran random forest regressions using the R package randomForestSRC [@ishwaran_fast_2023]. Random forests work by constructing multiple decision trees from bootstrapped samples of the dataset and averaging their predictions to improve accuracy and control overfitting. Each tree is built using a random subset of predictors at each split, enhancing diversity among trees. Variable importance was calculated by assessing the increase in prediction error when the values of a predictor were permuted. Variables' partial effect correspond to the impact of a single predictor on the response variable, keeping other variables fixed, to show how changes in the predictor influence the outcome. We used partial dependence plots to understand the relationship between predictors and the response.

In these random forests regression models we used the following descriptors : the event's deathtoll (absolute number), year of occurrence, country's Human Development Index (HDI), population (in millions) and population density (number of people per squared kilometer). Describing pages we use article length (number of characters, in the translated to English text), and locality (whether the article is written in a language local to the event's location).


## Output datasets and online app

The resulting tables can be consulted here:

[link to app](https://isig-apps.ens-lyon.fr/apps/lvaudor/glourb_floods_app/){target="_blank"}

The app provides a way to pick an event according to its characteristics and then consult in more detail the data associated with it (e.g. textual content and links to Wikidata and Wikipedia pages).
...
The initial query provided **`r length(unique(wd_events$flood))` Wikidata flood events**.

```{r show_precision_coords}
tiblocs=readRDS("data/precision_coords.RDS") %>% 
  mutate(ntot=sum(n)) %>% 
...
tiblocs %>% 
  knitr::kable()
```
In terms of spatial precision, most coordinates for floods are inferred from their locations (places where they occur such as cities, regions, and mainly countries), while few (`r tiblocs %>% filter(coords_from=="1) direct") %>% pull(starts_with("prop"))`%) come with direct coordinates.

```{r show_precision_dates}
tibdates=readRDS("data/dates_precision.RDS") %>% 
...
The date of most events is documented either directly or through the label of the event. Most events' dates are provided with day precision but an important proportion (about one third) is provided with only annual precision.
...
@fig-comp_WD_DFO .a confirms that compared to DFO, the proportion of events in Wikidata corresponding to countries with higher HDI is important. On the other hand, the distribution of events in the DFO dataset through HDI categories implies most events occur in the first and especially second quartile for HDI. An unbiased distribution of events (when defined through purely physical criteria) should reflect the total flood-prone area in each of in the four quartiles for HDI. An unbiased distribution of "socially significant" events would reflect both the total flood-prone area in each of the four quartiles for HDI (which probably represent vary heterogeneous values) but also the geographical effect of human distribution. The DFO dataset primarily takes into account physical criteria to define a flood but still incorporates societal elements (regarding damage and deathtoll for instance) to account for the actual importance or significance of the event. Hence we can assume that the DFO distribution of events provides a relatively unbiased estimate of floods distribution, at least compared to the WD distribution. This highlights the systematic difficulty of defining, detecting and reporting flood events in an unbiased, homogeneous way across projects.
...
wd_events_freq=wd_events %>% 
  mutate(cat_year=cut(year,breaks=c(1000,1200,1400,1600,1800,1900,1950,1985,
                                    2005,2023),dig.lab=10)) %>% 
...
wp_pages_complete=readRDS("data/wp_pages_complete.RDS") %>% 
  ...
  left_join(countries %>%
              select(country_label,HDI,population=population,area)) %>% 
  mutate(year=lubridate::year(date)) %>% 
...
We expect curation to be linked to the characteristics of the articles and the events they describe. We hypothesize in particular that curation will be higher for articles that are longer (editing efforts focused on longer articles), written in a language local to the event and that are about events with a high deathtoll (more public scrutiny). We also consider other explanatory variables such as time (as there can be varying standards for Wikipedia pages redaction), HDI (as a proxy to the proportion of internet users in the population), affected country's population and population density (as more public scrutiny and more people potentially physically affected by the flood could influence the curation process).
...
datarf=wp_pages_complete %>% 
      select(article,
           ...
           density,
           mean_views_f3months)
plot_random_forest(data=datarf,
                   response="curation",
                   remove="mean_views_f3months")
```

@fig-curation_causes *Links between curation of Wikipedia articles and a set of articles' descriptors (fushia), events' descriptors (green) and mixed descriptors (brown) assessed through a random forest regression. a) shows the relative importance of the descriptors in the prediction. The subsequent graphics show the partial effect of the descriptors on curation*

@fig-curation_causes .a shows that the level of curation of an article is primarily linked to other characteristics of the page, in particular the length of the article, and the article being written in an language local to the event. It is also linked to characteristics of the event, mainly year, HDI, density and deathtoll, and less importantly so to total population of the country where the event occurred. @fig-curation_causes .b shows that curation is positively correlated to articles length for articles below a certain size and then negatively for longer ones, possibly indicating that curation effort is focused on articles beyond a certain size (1000 to 10000 signs) but tend to be lower for the largest articles. Curation depends largely on the events' year of occurrence. Curation tend to be lower for events before 2000, probably due to the difficulty of providing sources (as urls mainly) for all statements, and increases rapidly after 2000, possibly due to the combination of an increase of accessible numerical sources and more enforcement of editing standards. Contrary to our expectation, curation is lower in languages local to the event. This could be due to a larger proportion of edits being done by local people motivated by a different set of goals compared to frequent Wikipedia contributors (e.g., aiming to add vernacular content rather than improving pre-existing content). HDI is the next most important explanatory variable, with higher levels corresponding to less curation, as we surmised, although there is quite a high variation of curation for the highest HDIs, possibly due to these representing few languages and thus being affected by Wikipedia language editions with varying standards. As expected, articles are all the more curated that the event's deathtoll is important, which reflects the fact that the level of scrutiny of Wikipedia editors on pages is correlated to the actual event's social impact. The density of the country where the event occurred is also positively correlated to the level of curation, while total population is negatively correlated. This might seem slightly contradictory with the fact that curation is lower for local events in the sense that editors being potentially directly affected by the event does not seem to favor curation. Hence this could actually reflect the fact that editors living in densely populated areas are more likely to be frequent Wikipedia contributors and thus more likely to be familiar with Wikipedia's editing standards.

We expect that the level of curation of an article will be correlated to the topics addressed in the article. We hypothesize that higher-quality articles will correspond to a higher level of curation and address topics which necessitate a deeper level of analysis (such as anticipation, governance, and hydrology) in higher proportions. We also expect higher-impacts events (in terms of deathtoll for instance) to cause this proportion to be higher, with complex issues around responsibility, causes, and adequacy of preventive measures only gaining attention when the damage is significant. To explore this relationship we sum the proportion in which these topics are addressed in each article and run a random forest regression model explaining this complex topics proportion. 

```{r, fig.width=10,fig.height=8, message=FALSE, warning=FALSE}
#| label: fig-complex_topics
#| fig-cap: "---"
wp_segments=readRDS("data/wp_segments.RDS")
tib_classes=wp_segments %>%
  select(class,class_name,color) %>%
...
tib=crossing(wp_pages %>% select(article) %>% unique(),
  ...
  unique() %>% 
  mutate(complex_topics=anticipation+governance+hydrology) %>% 
  select(article,complex_topics)
datarf=datarf %>% 
  left_join(tib,by="article")
plot_random_forest(data=datarf,
                   response="complex_topics",
                   remove="mean_views_f3months")
```


@fig-curation_topics *Links between proportion of complex topics in the Wikipedia articles and a set of articles' descriptors (fushia), events' descriptors (green) and mixed descriptors (brown) assessed through a random forest regression. a) shows the relative importance of the descriptors in the prediction. The subsequent graphics show the partial effect of the descriptors on the proportion*


@fig-curation_topics shows that the primary explanation to the proportion in which complex topics are addressed in the article is the length of the article (as we had already surmised from @fig-topic_per_length). The second one is the year of the event, with a high proportion of complex topics in the articles reporting older, historical events. That proportion decreases greatly from 1970 to 2015 and then starts increasing again, maybe due to increasing standards in the reporting of events. The proportion of complex topics addressed is also positively correlated to HDI. The proportion of complex topics is positively correlated to deathtoll for deathtolls up to 80, but negatively so beyond, reflecting that up to a certain point the discussion of causes and responsibility is increased by the event's damage severity but for higher deathtolls the narration of the event's unfolding in terms of weather and damage takes precedence. Contrary to our hypothesis also, articles with high levels of curation tend to address complex topics in lower proportions. This hints at Wikipedia's curation process actually tending to lower the proportion in which these topics appear (for instance through the suppression of unchecked or debatable statements). The effects of population and population density is difficult to interpret, as they probably result both from the number of people impacted and the variations in the standards of the people contributing the articles. The proportion of complex topics is lower for articles written in a local language, maybe reflecting the fact that providing documented statements about complex, potentially controversial topics surrounding floods is more difficult when one has more personal involvement and possibly less hindsight on an event.

### Number of views

We now consider only events which occurred after July 2015 (the date from which we have access to the number of views of Wikipedia pages), in order to be able to quantify the number of views of various articles at the same stage in time (i.e. from the moment the event occurred). 
...
We expect the short-term number of views of an article to be dependent of some of the articles' characteristics such as the article's being written in a language local to the event, article's length, and curation, although length and curation being measure on the latest state of development of articles rather than their state during their first three months of existence, they are probably to be interpreted more as consequences than causes. We expect that the number of views will be positively correlated to the level of curation of the article, as articles attracting more views are more likely to attract more scrutiny from editors. We expect the short-term number of views of pages to be higher in the local language, due to Wikipedia being used as an information source regarding the crisis unfolding and management. We also expect the number of views to reflect some characteristics of the events. For instance, we also expect the number of views to be higher for events with a higher deathtoll, and/or events occurring in areas where the density of population is particularly high, as these events are more likely to be of interest to a larger audience.

```{r fig-views,message=FALSE, warning=FALSE, fig.width=10,fig.height=8}
#| label: fig-views
#| fig-cap: "---"

plot_random_forest(data=datarf %>% filter(year>=2015),
                   response="mean_views_f3months")
```

@fig-views:*Links between short-term number of views of Wikipedia articles (during the 3 months following their creation) and a set of articles' descriptors (fushia), events' descriptors (green) and mixed descriptors (brown) assessed through a random forest regression. a) shows the relative importance of the descriptors in the prediction. The subsequent graphics show the partial effect of the descriptors on response.*

@fig-views .a shows that the short-term number of views of an article is most importantly correlated to curation, though not in the way we expected since highest number of views correspond to low levels of curation. As short-term number of views is also positively linked to article length, this could be interpreted as a sign that quantity of content (rather than quality) is the factor behind the number of views, especially shortly after a crisis, and that curation effort is not particularly focused on the articles generating the most views. The short-term number of views is also, as we expected, positively correlated to the events' deathtolls. The link to HDI is a bit more difficult to interpret with lowest and highest HDIs corresponding to the most views (independently of e.g. population density or deathtoll). We suspect the high number of views for lowest HDI could be due to e.g. Non-Governmental Organizations channelling foreign and digital-oriented attention to flood events in very vulnerable areas. The short-term number of views tends to increase over the years, and is higher for articles written in a local language, which was to be expected. The effect of population density is difficult to interpret, while total population is positively correlated to the short-term number of view, hinting at the sheer volume of viewers being linked to the size of national public audience. 
