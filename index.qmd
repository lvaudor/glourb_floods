...
We carried out a third Wikidata query for each country listed by the first query, to gather information about their coordinates, the languages used by their population (either official or used languages), and Human Development Index (HDI) \[ref\].
...
We tokenized the texts with the tidytext R package [@silge_tidytext_2016], lemmatized words and filtered out stop words based on the Iramuteq English dictionnary \[ref\]. We could then identify textual specificities based on various partitions of the corpus [@lafon_pierre_sur_1980, @loiseau_textometry_2022].
...
The initial query provided **`r nrow(wd_events)` Wikidata flood events**. In terms of spatial precision, most coordinates are inferred from either country or location (the major part of provided locations being countries too).
...
```{r plot_age_page_and_def_length_,fig.width=5,fig.height=4}
#| label: fig-age_page_and_def_length
#| fig-cap: "Relationship between age of the Wikipedia page (in days) and deficit in length. Older pages tend to correspond to a more important deficit in length: there is a tendency towards more synthesis as the page 'matures'. The residuals of this relationship is considered to be a way to quantify how controversial the edition of the page might have been."
tib=tib %>% 
