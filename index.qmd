...
@fig-curation_causes .a shows that the level of curation of an article is primarily linked to other characteristics of the page, (either as a cause or a consequence of them), in particular the number of views, the length of the article, and the article being written in an language local to the event. It is also linked to characteristics of the event, mainly deathtoll, and less importantly so to HDI, total population (in millions) of the country where the event occurred and density. Curation is negatively correlated to number of views. It is positively correlated to articles length for articles below a certain size and then negatively for longer ones, possibly indicating that curation effort is focused on longer articles. Curation tends to be lower in languages local to the event. This could be due to a larger proportion of edits being done by local people motivated by a different set of goals compared to frequent Wikipedia contributors (e.g., aiming to add vernacular content rather than improving pre-existing content). Articles are all the more curated that the event's deathtoll is important, which could reflect the fact that the level of scrutiny of Wikipedia editors on pages is correlated to the actual event's social impact. Older events exhibit less curation than recent ones, possibly due to increasing standards for the edition of pages and/or availability of other sources through time.
...
We expect the number of views of an article to be dependent of some of the articles' characteristics. In particular, we expect it to be more important right after the event occurred, and to decrease as time goes by. We expect that the number of views will be positively correlated to the level of curation of the article, as articles with a higher level of curation are more likely to be of higher quality, and hence more likely to be consulted. We expect the number of views of pages to be higher in the local language, in particular shortly after the event, due to Wikipedia being used as an information source regarding the crisis unfolding and management.
...
@fig-views_articles .a shows that the number of views of an article during the first three months of its existence is indeed significantly higher than in the long run. In both cases (short and long-term), the number of views is higher for local articles. @fig-views_articles .b shows that contrary to our expectation the number of views of an article, either during the first three months of its existence or long-term, is lower for articles with a higher level of curation. This is particularly the case for local articles, and during the first three months of the page's existence. This could be interpreted as a sign that quantity of content (rather than quality) is the factor behind the number of views, especially during a local crisis.

We also expect the number of views to reflect some characteristics of the events. For instance, we expect the number of views of pages to be higher for events occurring in countries with a higher HDI, as these countries are more likely to have a higher number of internet users. We also expect the number of views to be higher for events with a higher deathtoll, and/or events occurring in areas where the density of population is particularly high, as these events are more likely to be of interest to a larger audience. We use the ratio of population to area of the country where the event occurs (provided by Wikidata) as a rough estimate of density.
...
p2=ggplot(wp_pages_complete ,
       aes(x=density,y=mean_views_f3months, col=local))+
  ...
  scale_color_manual(values=c(col_DFO,col_WD))
p3=ggplot(wp_pages_complete ,
       aes(x=deathtoll,y=mean_views_f3months, col=local))+
...
rf_mean_views_f3months=plot_random_forest("mean_views_f3months")
```

Mean views during the 3 first months of page's existence.
