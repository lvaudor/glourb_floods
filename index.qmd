...
We collected Wikidata about floods, using the glitter R package (an API-client to the Wikidata SPARQL endpoint API) [@vaudor_glitter_2023].
...
We carried out a third Wikidata query for each country listed by the first query, to gather information about their coordinates, the languages used by their population (either official or used languages), and Human Development Index (HDI).
...
```{r plot_freq_events_in_datasets,fig.width=6,fig.height=3}
...
timebreaks=c(-Inf,2000,2010,2020,2024)
...
truc=wp_views_daily %>%
  ...
  mutate(prop=n/ntot)%>% 
  na.omit()
p1=ggplot(truc ,aes(x=delay,y=prop,fill=date_precision))+
  geom_col()+
...
@fig-delays shows that most of the writing of Wikipedia articles related to flood events take place during the month of their occurrence (except of course when Wikipedia did not exist (before 2001) or when it was still moderately used to document current events. This highlights the fact that Wikipedia articles about recent flood events are written as news rather than encyclopedic reports on past events. Views, on the other hand, are distributed more evenly through time. For instance, most views about events which occurred between 2010 and 2020 occur 5 to 10 years after the fact. 
...
@fig-curation_topics shows that articles with high levels of curation tend to deal with topics such as relief (diff=`r tib_summary %>% filter(class_name=="relief") %>% pull(diff)`, p-value=`r tib_summary %>% filter(class_name=="relief") %>% pull(signif) %>% round(5)`), and governance (diff=`r tib_summary %>% filter(class_name=="governance") %>% pull(diff)`, p-value=`r tib_summary %>% filter(class_name=="governance") %>% pull(signif) %>% round(5)`), while weather ((diff=`r tib_summary %>% filter(class_name=="weather") %>% pull(diff)`, p-value=`r tib_summary %>% filter(class_name=="weather") %>% pull(signif) %>% round(5)`)) is a bit less addressed. This hints at relief and governance being sensitive topics, which probably require and elicit more careful editing, discussion and fact-checking than e.g. reports on weather.
