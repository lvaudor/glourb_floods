...
compare_medians=function(tib,general=FALSE){
  ...
  if(general==TRUE){
      median_other=tib %>% filter(varx==FALSE) %>% pull(vary) %>% median(na.rm=TRUE)
      median_cat=tib %>% filter(varx==TRUE) %>% pull(vary) %>% median(na.rm=TRUE)
      result=median_cat-median_other
  }
  return(result)
}

calc_tib_summary=function(tib,group,y,x,round=3,diffgroupname="dg",diffxname="dx"){
  group=enquo(group)
  x=enquo(x)
  y=enquo(y)
  tibt=tib %>%
    ...
    mutate(varx:=!!x,
           ...
           group:=!!group) %>% 
    na.omit()
  f=function(x){return(tibt)}
  replace_varx=function(tib,xval){
    tib=tib %>% 
      mutate(varx=case_when(group==xval~TRUE,
                            TRUE~FALSE))
    return(tib)
  }
  tib1=tibt   %>%
    group_by(group) %>%
    tidyr::nest() %>% 
    mutate(data=purrr::map(group,~f(.x))) %>% 
    mutate(data=purrr::map2(data,group,~replace_varx(.x,.y))) %>% 
    mutate(median_general=purrr::map_dbl(data,~median(.x$vary,na.rm=TRUE)),
           signif_group=purrr::map_dbl(data,~wilcox.test(vary~varx, data=.x)$p.value),
           diff_group=purrr::map_dbl(data,~compare_medians(.x,general=TRUE))) %>% 
    mutate(signif_group_c=signif_stars(signif_group)) %>%
    mutate(label_group=paste0(diffgroupname,"=",round(diff_group,round),signif_group_c)) %>% 
    select(-data)
  ...
  tib2=tibt %>%
    ...
    tidyr::nest() %>% 
    mutate(
      median_group=purrr::map_dbl(data,~median(.x$vary,na.rm=TRUE)),
      signif_x=purrr::map_dbl(data,~wilcox.test(vary~varx, data=.x)$p.value),
      diff_x=purrr::map_dbl(data,~compare_medians(.x))) %>%
    mutate(signif_x_c=signif_stars(signif_x)) %>%
    mutate(label_x=paste0(diffxname,"=",round(diff_x,round),signif_x_c)) %>% 
    select(-data)
  tib=full_join(tib1,tib2,by=c("group"))
  tib3=tibt %>% 
    group_by(group,varx) %>% 
    tally()
  tib=full_join(tib,tib3,by=c("group"))
  return(tib)
  }
```
...
tib_effect=calc_tib_summary(deathtoll_compar,
                            group=paired_observations,
                            x=dataset, 
                            y=value,
                            round=10)
p1=ggplot(deathtoll_compar,
  aes(x=paired_observations, y=value))+
  geom_hline(data=tib_effect %>% select(median_general) %>% unique(),
             aes(yintercept=median_general),
             linewidth=2,alpha=0.5,col=col_DFO_WD)+
  geom_point(data=tib_effect %>% select(group,median_group) %>% unique(),
             aes(x=group,y=median_group),
             col=col_DFO_WD, shape=3,size=2)+
  geom_text(data=tib_effect %>% select(group,label_x) %>% unique(),
            aes(x=group,y=1000,label=label_x),
            position=position_dodge(width=0.8),
            size=3)+
  geom_text(data=tib_effect %>% select(group,label_group) %>% unique(),
          aes(x=group,y=100,label=label_group),
          position=position_dodge(width=0.8),
          size=3,col=col_DFO_WD)+
  geom_text(data=tib_effect %>% select(group,varx,n) %>% unique(),
        aes(x=group,group=varx,y=-0.5,label=n),
        position=position_dodge(width=0.8),
        size=2)+
...
*Distribution of flood events through time A) all events dated with a year-accuracy B) events dated with a year-accuracy starting in 2000*
...
*Time delay between events and a) revisions to the Wikipedia articles documenting them b) views of the Wikipedia articles. The delay might be negative (the edits or views about an event began before the recorded, average date for the event, hinting to an event with long duration or event with a low-precision recorded date). The color indicates the precision of the recorded event date (it can be accurate to the day, month or year).The grey rectangles indicate impossible-to-observe delays (first Wikipedia edits were in 2001 and first recorded views in 2015).*
...
p1=ggplot(tib, aes(x=catyear, curation))+
  geom_hline(aes(yintercept=unique(mean_curation_global)),
             col=col_DFO_WD,alpha=0.5,linewidth=2)+
  geom_boxplot(aes(fill=local))+
  geom_text(data=tib_mod,
            aes(y=-1,label=paste0("α=",round(diff,3),signifc)))+
  geom_point(data=tib_mod1,
             aes(x=catyear,y=mean_curation_local,fill=local),
             shape=3,size=3,stroke=2,
             position=position_dodge(width=0.8))+
  geom_point(data=tib_mod,
             aes(y=mean_curation),
             col=col_DFO_WD,shape=3,size=3,stroke=2)+
  
  geom_text(data=tib_mod1,
            aes(y=-0.8,fill=local,
                label=n),
            position=position_dodge(width=0.8))+
  geom_text(data=tib_mod2,col=col_DFO_WD,
            aes(y=1,label=paste0("δ=",round(Estimate,4),signifc)))+
...
p1=ggplot(views,aes(x=name,y=mean_views))+
  geom_hline(data=tib_effect,
             aes(yintercept=median_general),
             col=col_DFO_WD,linewidth=2,alpha=0.5)+
  geom_point(data=tib_effect,
             aes(x=group,y=median_group),
             col=col_DFO_WD, shape=3,size=2)+
  geom_text(data=tib_effect,
            aes(x=group,y=10000,label=label_x),
            position=position_dodge(width=0.8),
            size=3)+
  geom_text(data=tib_effect,
          aes(x=group,y=100000,label=label_group),
          position=position_dodge(width=0.8),
          size=3,col=col_DFO_WD)+
  geom_text(data=views_name_local,
            aes(x=name,group=local,y=5,label=n),
  ...
  scale_fill_manual(values=c(col_DFO,col_WD))
p2=ggplot(truc,aes(x=curation,y=mean_views,col=local))+
  ...
  scale_color_manual(values=c(col_DFO,col_WD))
p3=ggplot(views,aes(x=length,y=curation,col=local))+
  geom_point(alpha=0.5)+
  facet_grid(cols=vars(name))+
  scale_y_continuous(trans="log1p",
                     breaks=c(10,100,1000,10000,100000,1000000))+  scale_x_continuous(trans="log1p",
                     breaks=c(10,100,1000,10000,100000,1000000))+
  geom_smooth(method="lm")+
  scale_color_manual(values=c(col_DFO,col_WD))
...
*@fig-views_articles: Mean number of views during the first 3 months of the page's existence and long-term (after the 3 first months) according to the page's locality (a), and according to curation (b).*
