...
p2=ggplot(wm_dfo_comparison, 
  ...
  geom_boxplot()+
  geom_text(data=wm_dfo_comparison_summary,
            aes(x=HDI_class,y=-0.5,label=n),
...
```{r plot_deathtoll_comparison,fig.width=4,fig.height=5}
#| label: fig-deathtoll_WD_DFO
#| fig-cap: "a) Distribution of deathtolls estimates in Wikidata and DFO according to the estimates being paired or not (deathtoll estimates are paired when the events in Wikidata have been paired to an event in DFO). b) Comparison of deathtoll paired estimates in Wikidata and DFO for the same events. The symbol varies according to the deathtoll estimate in Wikidata being sourced, indicating a higher reliability of the estimate. "
deathtoll_compar=wm_dfo_comparison %>% 
  mutate(paired_observations=(!is.na(deathtoll) & !is.na(deathtoll_dfo))) %>% 
...
deathtoll_compar_summary=deathtoll_compar %>% 
  group_by(dataset,paired_observations) %>%
  summarise(value=mean(value,na.rm=TRUE))
p1=ggplot(deathtoll_compar,
  aes(x=dataset, y=value,fill=dataset))+
  geom_boxplot()+
  facet_grid(cols=vars(paired_observations))+
  geom_point(data=deathtoll_compar_summary,
  ...
  scale_fill_manual(values=c("dfo"="#607d73",
                             "wd"="#e7cee4"))+
  scale_color_manual(values=c("paired"="#9ecfbd",
                              "unpaired"="black"))+
...
p2=ggplot(wm_dfo_comparison %>%
       ...
       aes(x=deathtoll_wd,
           y =deathtoll_dfo))+
  geom_point(aes(shape=sourced),
             col="#9ecfbd",size=3, alpha=0.75)+
...
```{r wd_years, fig.width=8,fig.height=4, fig.show='hide'}
#| label: fig-wd_year
...
#| label: fig-delays
#| fig-cap: "Time delay (in years) between events and a) revisions to the Wikipedia articles documenting them b) views of the Wikipedia articles. The delay might be negative (the edits or views about an event began before the recorded, average date for the event, hinting to an event with long duration or event with a low-precision recorded date). The color indicates the precision of the recorded event date (it can be accurate to the day, month or year)."

timebreaks=c(-Inf,2000,2015,2020,2024)
...
truc=wp_views_daily %>%
  ...
  mutate(prop=n/ntot)%>% 
  na.omit()
imposs=cross_join(truc %>% group_by(year) %>% tally() %>% select(-n),
                  truc %>% group_by(delay) %>% tally() %>% select(-n)) %>% 
  mutate(impossible=case_when(year=="(-Inf,2000]" & delay<5~1,
                              year=="(2000,2015]" & delay>=11~1,
                              year=="(2015,2020]" & delay>=9~1,
  ...
  #                             year=="(2020,2024]" & delay>=8~1,
  #                             TRUE~0))
p1=ggplot(data=imposs,aes(x=delay,y=+Inf))+
  geom_col(aes(alpha=impossible))+
...
truc=wp_revisions %>%
  ...
  mutate(prop=n/ntot) %>% 
  na.omit()
p2=ggplot(truc ,aes(x=delay,y=prop,fill=date_precision))+
  geom_col()+
  facet_grid(rows=vars(year))+
  scale_x_continuous(breaks=(1:10)+0.5,
                     labels=labels)+
...
ggplot(tib,aes(x=class_name,y=prop))+
  ...
  coord_flip()+
  geom_point(data=tib_summary,aes(x=class_name,y=medianprop),col="red")+
  theme(legend.position="none")
```
...
wp_pages=left_join(wp_pages,tib %>% select(article,date_page,n_edits,rel_edit_length),
                   by="article",
                   relationship="many-to-many")
saveRDS(wp_pages,"data/wp_pages_synthesis.RDS")
p1=ggplot(tib,
       aes(x=n_edits,y=mean_edit_length,col=rel_edit_length))+
  geom_point()+
  scale_x_log10()+
  scale_y_log10()+
  geom_smooth(data=tib %>% filter(n_edits>=10),
              method="lm",
              col="dark grey")+
  scale_color_gradient2(low="green",mid="white",high="purple")

wp_pages2=wp_pages %>% 
  filter(!is.na(rel_edit_length),!is.na(length),lang!="simple")
wp_pages2=wp_pages2 %>% 
  mutate(fitted_rel_edit_length=predict(lm(rel_edit_length~log(length),data=wp_pages2),data=wp_pages2)) %>% 
  mutate(curation=rel_edit_length-fitted_rel_edit_length) %>% 
  mutate(curation=case_when(curation>0~curation/max(curation,na.rm=TRUE),
                             curation<=0~curation/max(-curation,na.rm=TRUE),
                              TRUE~curation))
wp_pages=wp_pages %>%
  left_join(wp_pages2 %>% select(article,curation),by=c("article"))
saveRDS(wp_pages,"data/wp_pages_curation.RDS")
library(ggnewscale)
p2=ggplot(wp_pages,
       aes(x=length,y=rel_edit_length,col=curation))+
  geom_point()+
  scale_x_log10()+
...
tib_summary=tib %>% 
  group_by(class_name) %>%
  tidyr::nest() %>% 
  mutate(signif=purrr::map_dbl(data,~wilcox.test(prop~highcuration, data=.x)$p.value),
         diff=purrr::map_dbl(data,
                             function(dat){dat %>%
                                 group_by(highcuration) %>% 
                                 summarise(x=median(prop)) %>%
                                 select(x) %>%
                                 pull() %>% 
                                 diff()})) %>% 
  mutate(signifc=case_when(signif<0.001~"***",
                          signif<0.01~"**",
                          signif<0.05~"*",
                          signif<0.1 ~".",
                          TRUE~"")) %>% 
  mutate(label=paste0("diff=",round(diff,3),signifc))
ggplot(tib,aes(x=class_name,fill=class_name,y=prop))+
  geom_boxplot(aes(fill=class_name,color=highcuration))+
  scale_fill_manual(breaks=tib_classes$class_name, values=tib_classes$color)+
  scale_color_manual(values=c("grey40","black"))+
  coord_flip()+
  geom_text(data=tib_summary,
            aes(x=class_name, y=-0.25,label=label),
             hjust=0)+
  guides(fill = FALSE)

```

@fig-curation_topics shows that articles with high levels of curation tend to deal with topics such as relief (diff=`r tib_summary %>% filter(class_name=="relief") %>% pull(diff) %>% round(3)`, p-value=`r tib_summary %>% filter(class_name=="relief") %>% pull(signif) %>% round(5)`), and governance (diff=`r tib_summary %>% filter(class_name=="governance") %>% pull(diff)%>% round(3)`, p-value=`r tib_summary %>% filter(class_name=="governance") %>% pull(signif) %>% round(5)`), while weather ((diff=`r tib_summary %>% filter(class_name=="weather") %>% pull(diff) %>% round(3)`, p-value=`r tib_summary %>% filter(class_name=="weather") %>% pull(signif) %>% round(5)`)) is a bit less addressed. This hints at relief and governance being sensitive topics, which probably require and elicit more careful editing, discussion and fact-checking than e.g. reports on weather.

```{r curation_causes, fig.width=8, fig.height=3}
#| label: fig-curation_causes
#| fig-cap: "a) Effect of the article being in a local language on curation. b) Effect of deathtoll and of the article being in a local language on curation." 
tt=wp_pages2 %>% 
  left_join(wm_full %>%
              select(flood,country,deathtoll,year) %>% 
              unique()) %>%
  left_join(countries %>% select(country,HDI) %>% unique()) %>% 
  mutate(year=cut(year,c(0,1950,1980,2000,2010,2020)))
lm1=lm(curation~local,data=tt)%>% summary()
lm2=lm(curation~local*log(deathtoll+1),data=tt) %>% summary()
p1=ggplot(tt,aes(x=local, y=curation,fill=local))+
  geom_boxplot()+theme(legend.position="none")+
  geom_text(data=tt %>% group_by(local) %>% tally(),
            aes(x=local, y = -1.2, label=paste0("n=",n)))
p2=ggplot(tt %>% filter(n_edits>10),
       aes(x=deathtoll,y=curation, col=local))+
...
@fig-curation_causes .a shows that the articles' being written in a local language (considering the event location) tends to be associated with a lower level of article curation (effect estimate= `r round(lm1$coef[2,1],3)`, t value=`r round(lm1$coef[2,3])`, Pr(\>\|t\|)=`r round(lm1$coef[2,4])`). This could be due to local articles being used more as information or managing crises tools than educational or encyclopedic carefully curated sources of knowledge. This hypothesis is reinforced by @fig-curation_causes .b which shows that the higher the deathtoll of an event, the less curated the local articles (slope for local articles `r round(lm2$coef[4,1],2)`, t value= `r round(lm2$coef[4,3],2)`, Pr(\>\|t\|)=`r round(lm2$coef[4,4],5)`).
